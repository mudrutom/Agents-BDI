...
public abstract class AbstractAgent {
	...
	/**
	 * Provides a easy way for the authentication against a server. It must be called before the agent is bind to the server and the
	 ...
	 * @throws java.io.IOException When the connection have not been initialized.
	 */
	...
	/**
	 * Waits for an authentication response from the server. It must be called after the <code>sendAuthentication</code> method call.
	 *
	...
	/**
	 * This method manages the reception of a packet from the server. It takes no parameters and suposes the authentication is done and hat
	 * succeed. It also writes to stderr the contents of the package.
	 *
	...
	/**
	 * Receives a packet from the server using the <code>receivePacket<code> method and converts the received data to a XML Document
	 * object.
	 *
	...
	/**
	 * Is the main agent's thread. It makes all the agent's work. First it manages the authentication, if it is not successful it will end.
	 * Then calls the <code>processLogin</code> method that is an user specified method. And next it remains in an infininte loop receiving
	 * and processing messages from the server. The messages must start with the <code>message<code> element. If it encounters any problem
	 ...
	 * <ol> <li><code>request-action</code></li> <li><code>sim-start</code></li> <li><code>sim-end</code></li> </ol>
	 * <p/>
	 * If the type is one of the first three, it builds a valid response envelop and calls the method related with the actual request wich
	 * will build the correct response content for the server. The responsible of sending such response is this method also, after it is
	 * builded.
	 *
